	...
	describe '#call' do
    	...
    	context 'the FSA api returns a list of authorities' do
			...
			it 'loads a list of authorities' do
				expect(subject.authorities_controller).to receive(:load_authorities).and_call_original	
				subject.call env
			end

			it 'displays the authorities' do
				expect(subject.authorities_controller).to receive(:load_authorities).and_return(mock_authorities)	
